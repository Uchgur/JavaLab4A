...
public class GraphicsDisplay extends JPanel {
	...
	protected void paintMarkers(Graphics2D canvas) {
		...
		for (Double[] point: graphicsData) {
			if (point[1] >= 0 && (Math.floor(point[1]) == Math.sqrt(Math.floor(point[1])) * Math.sqrt(Math.floor(point[1])) && Math.sqrt(Math.floor(point[1])) % 1 == 0)
					|| point[1] < 0 && (Math.ceil(point[1]) == Math.sqrt(Math.ceil(point[1])) * Math.sqrt(Math.ceil(point[1])) && Math.sqrt(Math.ceil(point[1])) % 1 == 0)) {
			...
			Rectangle2D.Double marker3 = new Rectangle2D.Double();
